...
import back_end.bookmakers.accounts_valuation as accs
...
def update_current_account_values(dir_name, data):
    accs.get_all_bookmakers_summary(data)
    io_utls.write_accounts_data(data, dir_name)

# Record accounts summary and total value.
def update_accs_summary(dir_name, data):
    noOfBookmakers = "Total number of bookmakers: " + str(accs.get_total_number_of_bookmakers(data))
    accountValue = "Accounts total value: " + str(accs.get_bookmakers_total_value(data))
...
def get_total_asset():
    ...
    accounts_data = accs.get_bookmakers_data()
